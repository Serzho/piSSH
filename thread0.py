...
NAME = 'pi'
PORT = '22'
client = piSSH.client(ip = IP, port = PORT, name = NAME)
pword = ''
'''
log('First thread was started at %s !!!' % time.asctime(time.localtime(time.time())))
...
def end():
    print('First thread find PASSWORD %s ' % pword)
    log('First thread find PASSWORD %s ' % pword)
    f = open('break.txt','w')
    f.write('True')
    f.close()
    f = open('temp.txt','w')
...
for b in range((len(a)//COUNTTHREADS)+1):
    #print(a[b])
    if(not check()):
...
'''
log('First thread was stoped at %s !!!' % time.asctime(time.localtime(time.time())))
